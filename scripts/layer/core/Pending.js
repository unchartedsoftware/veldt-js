(function() {
    ...
    const Pending = Base.extend({
        ...
        _increment: function(coord) {
            const hash = this._getTileHash(coord);
            if (this._pendingTiles[hash] === undefined) {
                this._pendingTiles[hash] = 1;
                const tiles = this._getTilesWithHash(hash);
                tiles.forEach(tile => {
        ...
        _getTileHash: function(coords) {
            const ncoords = this._getNormalizedCoords(coords);
            return ncoords.z + '-' + ncoords.x + '-' + ncoords.y;
        },

        _getTilesWithHash: function(hash) {
            const className = this._getTileClass(hash);
            const tiles = [];
        ...
        createTile: function(coord) {
            ...
            const tile = L.DomUtil.create('div', 'leaflet-tile leaflet-tile-pending');
            // get hash
            this._updateTile(coord, tile);
